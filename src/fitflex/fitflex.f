...
c	  n=numts*2+6
c
      parameter (numbt=66160,numgr=143754,numts=11,nc1=30,nc2=9,
     + m=209914, n=28, nc=39)
...
90      enddo
        amatr(i,2*numts+1)=xbb(i)*(111195/60.)/sdbtdat(i)
        amatr(i,2*numts+2)=ybb(i)*(111195/60.)/sdbtdat(i)
...
110     enddo
        amatr(i+numbt,2*numts+1)=0.0
        amatr(i+numbt,2*numts+2)=0.0
        amatr(i+numbt,2*numts+3)=0.0
        amatr(i+numbt,2*numts+4)=xgr(i)*(111195/60.)/sdgrdat(i)
        amatr(i+numbt,2*numts+5)=ygr(i)*(111195/60.)/sdgrdat(i)
        amatr(i+numbt,2*numts+6)=1.0/sdgrdat(i)
