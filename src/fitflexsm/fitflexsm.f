...
c     Define variables
      ...
      integer numbt, numgr, numts, numts2, nc, nc1, nc2, m, n
...
c
c	  because of the smoothing,
c	  m=numbt+numgr+numts	 
c	  because of the nuisance parameters for fitting a plane
c	  trend to the bathymetry and gravity data,	
c	  n=numts*2+6
c
      parameter (numbt=215631,numgr=215631,numts=5,
     + m=431627, n=16, nc=39)

c      parameter (numbt=83053,numgr=83053,numts=5,
c     + m=166111, n=16, nc=39)
...
c     Use the topo Green's functions
      do 80 i=1, numbt
        do 90 j=1, numts
...
90      enddo
        amatr(i,numts2+1)=xbb(i)*(111195/60.)/sdbtdat(i)
        amatr(i,numts2+2)=ybb(i)*(111195/60.)/sdbtdat(i)
        amatr(i,numts2+3)=1.0/sdgrdat(i)
        amatr(i,numts2+4)=0.0
        amatr(i,numts2+5)=0.0
        amatr(i,numts2+6)=0.0

80    enddo

c     Use the grav Green's functions
      do 100 i=1, numgr
        do 110 j=1, numts
          amatr(i+numbt,j)=ggm(i,j)/(sdgrdat(i)*wgrav)
          amatr(i+numbt,j+numts)=ggv(i,j)/(sdgrdat(i)*wgrav)
110     enddo
        amatr(i+numbt,numts2+1)=0.0
        amatr(i+numbt,numts2+2)=0.0
        amatr(i+numbt,numts2+3)=0.0
        amatr(i+numbt,numts2+4)=xgr(i)*(111195/60.)/sdgrdat(i)
        amatr(i+numbt,numts2+5)=ygr(i)*(111195/60.)/sdgrdat(i)
        amatr(i+numbt,numts2+6)=1.0/sdgrdat(i)

100   enddo

c     Construct smoothing matrices
c	  Recall that amatr was initialized to zero
c     We are using backward differences      
      
      do 120 i=2, numts
          amatr(i+numbt+numgr,i-1)=-1.0
          amatr(i+numbt+numgr,i)=1.0
          amatr(i+numbt+numgr,i+numts-1)=-1.0
          amatr(i+numbt+numgr,i+numts)=1.0
120   enddo
...
c     Use the grav data
        do 320 i=1, numts
c	  Actually unnecessary because bvec was initialized to zero
          bvec(i+numbt+numgr)=0.0
...
510     enddo
        ptopo(i)=ptopo(i)+(tl(numts2+1)*xbb(i))
     +                   +(tl(numts2+2)*ybb(i))+(tl(numts2+3))  
500   enddo
...
530     enddo
        pgrav(i)=pgrav(i)+(tl(numts2+4)*xbb(i))
     +                   +(tl(numts2+5)*ybb(i))+(tl(numts2+6))
520   enddo
