...
c
      ...
      real*8 fz(nj,ni),fpeek(nj,ni)
      real*8 D(nj,ni),b(nj,ni),bxy(nj,ni)
      real*8 wold(nj,ni),wnew(nj,ni)
      real*8 gz(nj,ni),dp(nj,ni)
      real*8 grav(nj,ni)
      real*8 dtx(nj,ni), dtxy(nj,ni), dty(nj,ni) 
      real*8 wtx(nj,ni), wtxy(nj,ni), wty(nj,ni)
      real*8 xwind(nj),ywind(ni)
      complex*16 fkz(nj2,ni),gkz(nj2,ni)
      complex*16 wconstk(nj2,ni),wpertk(nj2,ni)
      complex*16 woldk(nj2,ni),wnewk(nj2,ni)
      complex*16 Dk(nj2,ni),bk(nj2,ni), bxyk(nj2,ni)
      complex*16 gravk(nj2,ni)
      complex*16 dpk(nj2,ni) 
      complex*16 dtxk(nj2,ni), dtxyk(nj2,ni), dtyk(nj2,ni)
      complex*16 wtk(nj2,ni) 
      complex*16 wtxk(nj2,ni), wtxyk(nj2,ni), wtyk(nj2,ni)
      complex*16 fzval,gzval,tkval,grval,wctemp,wdtemp
      complex*16 gsurf,gmoho
      dimension n(2)
...
c      fz(j,i)=fz(j,i)*xwind(j)*ywind(i)/(ni*nj)
       fz(j,i)=fz(j,i)/(ni*nj)
       D(j,i)=(D(j,i)-D0)*xwind(j)*ywind(i)
       dp(j,i)=D(j,i)
       wold(j,i)=0.0
  120 continue
c
...
c
      call fourt(dp,n,2,1,-1,work,nwork)
...
c
      wpertk(j,i)=(beta**2*bk(j,i)-((2*pi)**4*bxyk(j,i)*(1-rnu)))/den
      if(icount.eq.0) then
