...
c
      ...
      real*4 topo(nj*ni),rigid(nj*ni),curv(nj*ni)
      real*4 fz(nj,ni)
...
c
      ...
      if(narg.lt.6) then
        write(*,'(a)')'  '
        write(*,'(a)')
     & 'Usage: flex2d topo.grd rig.grd isurf w.grd curv.grd grav.grd'
        write(*,'(a)')
        write(*,'(a)')
...
c need to reveres the sign of fz so positive load produces pos. topo.
c
      do 120 i=1,ni
      do 120 j=1,nj
       k=nj1*(i-1)+j
...
c
     ...
     +              trim(cgrav)//char(0),trim(cgrav)//char(0))
      call writegrd(curv,nj1,ni1,y0,x0,dx,dy,rland,rdum,
