...
markersize = 20;  %size of line markers, default is 6
...
%     (strcmpi(get(axesh(i), 'tag'), 'legend') == false);
    if (strcmpi(get(axesh(i), 'tag'), 'legend') == false)
        ...
        set(curAx, 'FontName', legendfont)
        set(curAx, 'FontSize', legendfontsize)
        set(curAx, 'FontWeight', legendfontweight)
        set(curAx, 'FontAngle', legendfontitalics)
    end
end
end
